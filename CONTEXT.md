...
> refreshed 2026-09-25 | upstream default: main @ b578eab
...
- 2026-09-25 trivial-fix pass — 5 verified fixes bundled into ONE docs PR (fork PR #19): (1) project-management.md release-notes link blob/mains/docs/topics -> blob/main/docs/community (mains typo + stale path, 404); (2) release-notes.md contributor @maerteijn 404 -> @mj026 (PR #9198 author); (3) release-notes.md malformed [@james](github.com/james)-mchugh -> [@james-mchugh](github.com/james-mchugh) (PR #9455 author); (4) release-notes.md malformed [@p](github.com/p)-schlickmann -> [@p-schlickmann](github.com/p-schlickmann) (PR #9480 author); (5) parsers.md quote attribution typo 'Malcom Tredinnick' -> 'Malcolm Tredinnick'. NOTE: fixes (1)-(4) are the SAME links the 2026-09-03 / 2026-09-09 fork passes fixed in PR #2/#10, which were CLOSED when an unrelated upstream PR (#10038) merged — those fixes NEVER LANDED upstream and all four links are still broken in current upstream main (verified live). Re-submitted this run because the gaps genuinely persist; each replacement verified 200 via HTTP/API. Docs-only, 3 files, <= max_diff_lines, meaning-preserving, no churn. Fork CI: same persistent fork artifact (0 workflows register).
## Mined gaps (discovered, not yet attempted)
- 2026-09-24 OrderingFilter `__all__` annotation labels — self-found, ATTEMPTED (fix + test, fork PR #18). No other repo-audit findings warrant a second pick this run.
